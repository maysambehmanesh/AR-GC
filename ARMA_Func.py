...
def computeLoaderCombine5(data_tr,epochs):
    ...
    return newLoader_tr,L



def computeLoaderCombine4(data_tr,appriximate_phsi,N_scales,scales,m,epochs,thr):
...
def computeLoaderCombine3(dataset,mask_tr,mask_va,mask_te,appriximate_phsi,N_scales,scales,m,epochs,thr):
    ...
    # store x,y, and a
    x.append(itrData[0][0])    # feature vector
    y.append(data_y)           # labels
    a.append(adj)              # adjacency matrix
    
    # convert adjacency     
    adj2=tf.sparse.to_dense(adj)
    adj2=adj2.numpy()
    adj3=scipy.sparse.csr_matrix(adj2)

    
    # compute laplacian
    L = laplacian3(adj3,normalized=True)

    del adj,adj2,adj3
    
    N_data=len(data_adj[0])
    
    # compute psi and psiInv
    weight_sparse=True
    weight_normalize=False

    if appriximate_phsi == True:
        psi0,psi_inv0=approximate_Psi(L,N_scales,m)
        # psi0,psi_inv0=approximate_Psi2(adj2,1,m,N_data)
    else:
        psi0,psi_inv0=compute_Psi2(L,scales,weight_sparse,weight_normalize,thr)
        
    #store psi and PsiInv
    psi.append(psi0[0])
    psi_inv.append(psi_inv0[0])
        

       
        
    # convert psi and psiInv to array        
    psi=np.asarray(psi)
    psi_inv=np.asarray(psi_inv)
    
    # check sparsity of psi
    psi[psi<thr]=0
    psi_inv[psi_inv<thr]=0
    ...
    itrData_tr=changeTupleItem(itrData,0,data_adj)
    
    #  change y with list of y in multimodal
    # y=tuple(y)
    # itrData_tr=changeTupleItem(itrData_tr,1,y)
    # 
    
    
    itrData_va=changeTupleItem(itrData_tr,2,mask_va)
    
    itrData_te=changeTupleItem(itrData_tr,2,mask_te)
    ...
    # aa2=list(newLoader.as_numpy_iterator())
    
    return newLoader_tr,newLoader_va,newLoader_te

def computeLoaderCombine2(data_tr,loader_tr,loader_te,appriximate_phsi,N_scales,scales,m,epochs,thr):
    ...
    return newLoader_tr,newLoader_te
    
def get_phsi(dataset,appriximate_phsi,N_scales,scales,m,epochs,thr):
    ...
    """
    itrData_tr=data_loader2(dataset,dataset)
    # itrData_tr=data.__next__()
    ...
    if appriximate_phsi == True:
        ...
        psi,psi_inv=compute_Psi3(L,scales)
